...
class MemberFilter(FilterSet):
    ...
    def full_text_search(self, queryset, name, value):
        queryset = queryset.filter(Q(name__icontains = value) |  Q(id__icontains = value))
        # queryset = queryset.annotate(search=SearchVector('body_text')).filter(search=value)
        
        # queryset = queryset.annotate(
        #         ts=RawSQL(

        #             params=[],
        #             output_field=SearchVectorField("name","phone","email")
        #         )
        #     ).filter(
        #         ts=value
        #     )
